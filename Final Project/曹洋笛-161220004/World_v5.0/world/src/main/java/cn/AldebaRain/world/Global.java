...
public class Global {
	...
	/**	默认保存文件	*/
	public static final String defaultFilename = Global.class.getClassLoader()
			.getResource("records/defaultRecordFile.xml").getFile();

	/**	测试用文件（不可更改）	*/
	public static final String defaultTestFilename = Global.class.getClassLoader()
			.getResource("records/JUnitTestRecordFile.xml").getFile();

	/**	默认文件选择器初始路径	*/
	public static final String defaultPath() {
		String targetClassesPath = Global.class.getClassLoader().getResource("").getPath();
		String myPath = targetClassesPath.substring(0, targetClassesPath.length() - 15) + "myRecords";
