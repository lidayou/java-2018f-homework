...
public class YanXing implements Formation{
    ...
    public void SetTites(Coordinate[][] Field, int x, int y, Tite[] tites) {
        ...
        while(!(x>=6&&y<=8)){
            ...
            System.out.println("请重新输入雁行领队的横坐标: ");
            x=in.nextInt();
            System.out.println("请重新输入雁行领队的纵坐标:  ");
            y=in.nextInt();
        }
        ...
        Field[x-5][y+5].tite=tites[5];
        Field[x-5][y+5].empty=false;
        Field[x-6][y+6].tite=tites[6];
        Field[x-6][y+6].empty=false;
    }
}
