...
class CalabashSide extends BattleArray {
    ...
    private void initCalabashGround(BattleField battleField) {
    ...
    public void addLeader(BattleField battleField) {
        int X = battleField.scale();
        int Y = X / 2 - 1;
    ...
    public void snakeArray(BattleField battleField) {
        int X = battleField.scale();
        int Y = X / 2 - 1;
...
class MonsterSide extends BattleArray {
    ...
    private void initMonsterGround(BattleField battleField) {
        int X = battleField.scale();
        int Y = X / 2;
        for (int i = 0; i < X; ++i)
            for (int j = Y; j < X; ++j)
    ...
    public void craneArray(BattleField battleField) {
        int X = battleField.scale();
        int Y = X / 2;
    ...
    public void arrowArray(BattleField battleField) {
        int X = battleField.scale();
        int Y = X / 2;
        initMonsterGround(battleField);
        battleField.addBeing(soldiers[0], Y, Y);
